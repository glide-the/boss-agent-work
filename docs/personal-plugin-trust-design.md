...
分支：`codex/chrome-dialog-cancellation`
...
本次修复使用插件自带的 MCP `boss_repl`，由它启动独立的 `node_repl` 子进程，并仅为该子进程注册插件自己的 `boss_browser` trusted service。Browser Client 通过 `nodeRepl.rpc("boss_browser", ...)` 调用服务；服务内部保留现有 `nativePipe -> Native Host -> Chrome Extension` 通信。启动器主动从子进程环境删除旧 SHA allowlist，不修改用户全局配置，也不覆盖官方 `browser`、官方插件、代码签名或企业策略。

## 证据状态

| 项目 | 状态与证据 |
| --- | --- |
| 插件身份 | confirmed：`chrome-dev@codex-chrome-automation-local`；修改前版本 `26.707.30751-standalone.4`，本次最终版本 `.7`。名称、marketplace 与版本来自插件 manifest 和用户配置。 |
...
| Desktop 缓存 | inferred：已运行的 Desktop 不会自动发现新插件 MCP；安装后需重新加载插件或完整重启。真实新会话验证前不宣称连接恢复。 |
...
配置冲突、损坏 registry、Host 所有权不一致或托管配置都会在写入前退出。Native Host manifest、registry、Host config 与 `latest` 状态先保存独立快照；回滚仅在目标没有后续第三方修改时恢复。`config.toml` 由官方 plugin installer 管理，reconcile 只读，因而不会覆盖其他插件和信任记录。
...
- `NATIVE_HOST_REGISTRATION_ERROR` / `CONFIG_CONFLICT`：保留原文件，按输出的快照路径恢复。
...
sequenceDiagram
    ...
    alt 未安装或配置冲突
        ...
        alt 服务未授权或运行时不兼容
            ...
            I->>I: 预检冲突并备份用户级 Host 目标
            I->>N: 注册或幂等核对 Native Host
            alt 写入失败
                I-->>U: NATIVE_HOST_REGISTRATION_ERROR + 回滚快照
            else 注册完成
...
| 配置安全 | 通过 | config 只读；Host 目标结构化最小写入并先备份；冲突退出。 |
...
`scripts-bak` 在修改前是唯一不可变基线，digest 为 `85d1bc4f7d456ef75eceab7df6159ebfdfce23de0af51175ec9ba2dc9e579964`（344 文件）。部署新增 service/launcher 时未覆盖该目录，`bun run rollback` 会删除新增生成物并恢复原树。用户级 Host 回滚使用初始化输出的 snapshot 文件。

尚不能证明的事项：完整重启后 Desktop 是否即时列出 `boss_repl`；真实 Chrome 扩展当前是否在线；Windows HKCU 回滚的完整性；未出现在本机文件中的远程企业策略。这些必须按真实结果标注，不能从服务 ping 推断。
