...
## P6 首次回复可发简历，但同一会话只能发一次

发起会话后，只要 Boss 已有第一条回复，就可以通过工具栏「发简历」发送，不必等待正式请求卡片。

强制规则：
1. 发送前扫描会话成功系统消息，并查 `data/reply_progress.jsonl`；任一处已有发送证据时立即跳过。
2. 无请求卡时只用 `sendResumeOnceAfterBossReply()`；有请求卡时用 `agreeAndSendResume()`，两者都内置同一去重检查。
3. 工具栏仍为 `unable` 时不得强点，也不得声称已发送；记录页面状态后继续处理文字回复。
4. 后续 Boss 再次回复或再次索要简历，不构成第二次发送理由。
