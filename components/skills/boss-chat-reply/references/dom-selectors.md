...
- 误拒系统消息：`您已拒绝向对方发送简历`
- 工具栏「发简历」按钮：`.chat-conversation div.toolbar-btn, .chat-editor div.toolbar-btn` 中严格文本等于 `发简历` 的元素；发起会话后 Boss 首次回复通常解锁，`unable` 表示当前不可发送
- 去重成功文本：会话中出现 `已发送给Boss`、`附件简历已发送给对方` 或等价系统消息，视为该会话已经发送过简历
