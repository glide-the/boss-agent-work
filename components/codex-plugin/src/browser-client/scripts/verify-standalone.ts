...
function nestedRecord(value: unknown, property: string): JsonRecord | null {
  return objectRecord(objectRecord(value)?.[property]);
}

function readJson(filePath: string): JsonRecord {
  const parsed = objectRecord(JSON.parse(readFileSync(filePath, "utf8")));
  if (parsed == null) throw new Error(`Expected a JSON object in ${filePath}.`);
  return parsed;
}

function readIdentity(filePath: string): StandaloneIdentity {
  const parsed = readJson(filePath);
  const extensionHostNames = parsed.extensionHostNames;
  if (
    typeof parsed.extensionHostName !== "string"
    || !Array.isArray(extensionHostNames)
    || !extensionHostNames.every((value) => typeof value === "string")
    || typeof parsed.extensionId !== "string"
    || typeof parsed.marketplaceName !== "string"
    || typeof parsed.pluginName !== "string"
    || typeof parsed.proxyHost !== "string"
  ) {
...
export function deriveChromeExtensionId(publicKey: string): string {
  ...
  return digest
    .slice(0, 32)
    .replace(/[0-9a-f]/gu, (character) => "abcdefghijklmnop"[Number.parseInt(character, 16)]!);
}
...
export function verifyStandalone({
  ...
  const expectIncludes = (label: string, source: string, value: string): void => {
    if (!source.includes(value)) failures.push(`${label}: missing ${JSON.stringify(value)}`);
  };
  const expectExcludes = (label: string, source: string, value: string): void => {
  ...
  expectEqual(
    "manifest-derived extension ID",
    typeof extensionKey === "string" ? deriveChromeExtensionId(extensionKey) : undefined,
  ...
  const installerHostNames: string[] = installSource.match(/com\.openai\.codexextension(?:\.dev|\.internal)?/gu) ?? [];
  const extensionHostNames: string[] = backgroundSource.match(/com\.openai\.codexextension(?:\.dev|\.internal)?/gu) ?? [];
  ...
  const extensionHasChannelMap = extensionHostNames.includes("com.openai.codexextension")
  ...
  if (liveMode) {
    if (process.platform !== "darwin") {
      ...
      if (!existsSync(liveManifestPath)) {
        ...
        expectEqual(
          "live allowed origin",
          Array.isArray(origins) && origins.includes(`chrome-extension://${identity.extensionId}/`),
          true,
        );
        ...
        if (!existsSync(registryPath)) {
          ...
          const matchingEntry = Array.isArray(entries)
            ? entries.map(objectRecord).find((entry) => {
              const extensionIds = entry?.extensionIds;
              const nativeHostNames = entry?.nativeHostNames;
              return Array.isArray(extensionIds)
                && extensionIds.includes(identity.extensionId)
                && Array.isArray(nativeHostNames)
                && nativeHostNames.includes(identity.extensionHostName);
            })
            : undefined;
