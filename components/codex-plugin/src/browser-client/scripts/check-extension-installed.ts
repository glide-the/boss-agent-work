...
// Evidence source: components/codex-plugin/scripts-bak/check-extension-installed.js
...
interface ExtensionProfileStatus {
...
interface SelectedExtensionProfileStatus extends ExtensionProfileStatus {
  selected: boolean;
}

interface ExtensionInstallStatus {
  extensionId: string;
  userDataDirectory: string;
  selectedProfileDirectory: string;
  installed: boolean;
  enabled: boolean;
  profiles: SelectedExtensionProfileStatus[];
  exitCode: number;
}

interface ExitCodeInput {
  enabled: boolean;
  installed: boolean;
}
...
function getChromePreferencesPathOverride(): string | null {
...
function loadRemoteChromeExtensionId(): string {
...
function getChromeExtensionInstallStatus(): ExtensionInstallStatus {
  const extensionId = loadRemoteChromeExtensionId();
...
function getChromeExtensionPreferences(
  profilePath: string,
  extensionId: string,
...
function getExitCode({ enabled, installed }: ExitCodeInput): number {
