...
function hasIntegratedPolicy(source: string): boolean {
  return source.includes(IMPORT_LINE)
    && source.includes("new __BossSiteStatusPolicy")
    && source.includes(".throwIfBlocksUrl(");
}
...
export function patchBrowserClientSource(input: string): PatchResult {
...
export function verifyPatchedBrowserClientSource(source: string): string[] {
  const failures: string[] = [];
  if (!source.includes(IMPORT_LINE)) failures.push("site-status-policy import is missing");
  if (!hasIntegratedPolicy(source)) failures.push("site status policy integration is missing");
