...
export function resolveSiteStatusConfiguration(environment = {}) {
...
export function buildSiteStatusRequest(targetUrl, {
  baseUrl,
  conversationId,
  turnId,
...
export function parseSiteStatusResponse(value) {
  const agent = value?.feature_status?.agent;
...
function errorCode(error) {
  if (error?.name === "AbortError" || error?.code === "site_status_timeout") return "timeout";
...
export class SiteStatusPolicy {
  constructor({
    ...
    getEnvironment = () => globalThis.nodeRepl?.env ?? process.env,
    getFetch = () => globalThis.nodeRepl?.fetch ?? globalThis.fetch,
