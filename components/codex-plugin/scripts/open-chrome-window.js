...
// scripts/open-chrome-window.ts
var CHROME_PREFERENCES_PATH_ENV = "CODEX_CHROME_PREFERENCES_PATH";
var CHROME_USER_DATA_DIR_ENV = "CODEX_CHROME_USER_DATA_DIR";
var MACOS_CHROME_BUNDLE_ID = "com.google.Chrome";
var MACOS_CHROME_APP_NAMES = ["Google Chrome.app"];
var WINDOWS_CHROME_EXECUTABLE = "chrome.exe";
...
function usage() {
  ...
  console.error("");
  console.error(`Optional profile-root override: ${CHROME_USER_DATA_DIR_ENV}=/tmp/chrome-root`);
  console.error(`Optional preferences-file override: ${CHROME_PREFERENCES_PATH_ENV}=/tmp/Profile/Preferences`);
}
...
function isChromeApp(appPath, allowNamedFallback = false) {
  ...
  if (info.CFBundleIdentifier === MACOS_CHROME_BUNDLE_ID)
    return true;
  return allowNamedFallback && MACOS_CHROME_APP_NAMES.includes(path.basename(appPath));
}
function findMacosChromeAppPath() {
  for (const appPath of macosAppCandidates())
    if (isChromeApp(appPath, true))
      return appPath;
  for (const appPath of mdfindChromeApps())
    if (isChromeApp(appPath))
      return appPath;
  for (const baseDir of macosAppSearchDirs()) {
...
function windowsChromeInstallPaths() {
  ...
  const candidates = appPathKeys.map((keyPath) => readWindowsRegistryValue(keyPath, null)).filter(isString);
