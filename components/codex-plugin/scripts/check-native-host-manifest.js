...
// scripts/check-native-host-manifest.ts
var fs;
var os;
var path;
var process = globalThis.process;
var execFileSync;
var expectedExtensionId = "";
var expectedHostName = "";
var CHROME_NATIVE_HOST_MANIFEST_PATH_ENV = "CODEX_CHROME_NATIVE_HOST_MANIFEST_PATH";
var CHROME_EXTENSION_ID_CONFIG_FILENAME = "extension-id.json";
var WINDOWS_NATIVE_HOST_REGISTRY_KEY_PREFIX = "HKCU\\Software\\Google\\Chrome\\NativeMessagingHosts";
function usage() {
  console.error("Usage: scripts/check-native-host-manifest.js [--json]");
  console.error("");
  console.error(`Expected extension ID is read from scripts/${CHROME_EXTENSION_ID_CONFIG_FILENAME}.`);
  console.error(`Optional manifest-file override: ${CHROME_NATIVE_HOST_MANIFEST_PATH_ENV}=/path/to/native-host.json`);
}
function getNativeHostManifestLocation() {
  if (process.env[CHROME_NATIVE_HOST_MANIFEST_PATH_ENV]) {
    return {
      manifestPath: path.resolve(process.env[CHROME_NATIVE_HOST_MANIFEST_PATH_ENV]),
      registryKey: null,
  ...
  if (process.platform === "darwin") {
    return {
      manifestPath: path.join(os.homedir(), "Library", "Application Support", "Google", "Chrome", "NativeMessagingHosts", `${expectedHostName}.json`),
      registryKey: null,
      registryManifestPath: null,
      registryKeyExists: null
    };
  ...
  throw new Error(`Unsupported platform for native host manifest check: ${process.platform}. This script supports macOS and Windows.`);
}
function getDefaultWindowsManifestPath() {
  return path.join(os.homedir(), "AppData", "Local", "OpenAI", "extension", `${expectedHostName}.json`);
}
...
function getNativeHostManifestLocationProblem(location, manifestExists) {
  ...
  if (location.registryKeyExists === false) {
    problems.push(`Windows native host registry key does not exist: ${location.registryKey}`);
  }
  if (!manifestExists) {
    problems.push(`Native host manifest does not exist: ${location.manifestPath}`);
  }
...
function readNativeHostManifest(filePath) {
  try {
    ...
    throw new Error(`Could not read native host manifest ${filePath}: ${error instanceof Error ? error.message : String(error)}`, { cause: error });
  }
}
...
function loadExpectedChromeExtensionConfig() {
  ...
  return {
    ...
    ...typeof config.extensionHostName === "string" ? { extensionHostName: config.extensionHostName } : {}
  };
}
function loadExpectedHostName() {
  const config = loadExpectedChromeExtensionConfig();
  if (typeof config.extensionHostName === "string" && config.extensionHostName.trim() !== "") {
...
function getNativeHostManifestStatus() {
  ...
  const locationProblem = getNativeHostManifestLocationProblem(location, exists);
  if (locationProblem) {
    return {
  ...
  const registryMatchesManifestPath = location.registryManifestPath == null || path.resolve(location.registryManifestPath) === path.resolve(location.manifestPath);
  const correct = nameMatches && hasExpectedOrigin && registryMatchesManifestPath;
  return {
    manifestPath: location.manifestPath,
    registryKey: location.registryKey,
    registryManifestPath: location.registryManifestPath,
    ...
    correct,
    problem: correct ? null : describeManifestProblem({
      nameMatches,
      hasExpectedOrigin,
      registryMatchesManifestPath
    })
...
function describeManifestProblem({
  ...
  if (!hasExpectedOrigin) {
    problems.push(`allowed_origins does not include chrome-extension://${expectedExtensionId}/`);
  }
...
function main() {
  ...
  if (json)
    ...
    if (result.registryManifestPath) {
      console.log(`Windows registry manifest path: ${result.registryManifestPath}`);
    }
...
try {
  ...
  ]).then(([fsModule, osModule, pathModule, processModule, childProcessModule]) => {
    fs = fsModule;
    os = osModule;
    path = pathModule;
