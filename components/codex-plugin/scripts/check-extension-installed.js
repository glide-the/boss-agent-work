...
// scripts/check-extension-installed.ts
var fs;
var os;
var path;
var process = globalThis.process;
var CHROME_PREFERENCES_PATH_ENV = "CODEX_CHROME_PREFERENCES_PATH";
var CHROME_USER_DATA_DIR_ENV = "CODEX_CHROME_USER_DATA_DIR";
var CHROME_EXTENSION_ID_CONFIG_FILENAME = "extension-id.json";
var EXIT_INSTALLED_AND_ENABLED = 0;
var EXIT_INSTALLED_NOT_ENABLED = 1;
var EXIT_NOT_INSTALLED = 2;
var EXIT_RUNTIME_ERROR = 3;
...
function usage() {
  ...
  console.error("");
  console.error(`Default extension ID is configured in scripts/${CHROME_EXTENSION_ID_CONFIG_FILENAME} (${extensionId}).`);
  console.error(`Optional profile-root override: ${CHROME_USER_DATA_DIR_ENV}=/path/to/chrome-root`);
  console.error(`Optional preferences-file override: ${CHROME_PREFERENCES_PATH_ENV}=/path/to/Profile/Preferences`);
}
...
function loadRemoteChromeExtensionId() {
  const configPath = resolveSiblingScriptPath(CHROME_EXTENSION_ID_CONFIG_FILENAME);
...
function resolveChromeProfileDirectory(userDataDirectory) {
  const localStateProfile = resolveChromeProfileDirectoryFromLocalState(userDataDirectory);
  if (localStateProfile)
    return localStateProfile;
  const latestProfile = findLatestChromeProfile(userDataDirectory);
  if (latestProfile)
    return latestProfile;
  throw new Error(`Could not find a Chrome profile directory with Preferences in ${userDataDirectory}.`);
}
...
function findLatestChromeProfile(userDataDirectory) {
  if (!fs.existsSync(userDataDirectory)) {
    throw new Error(`Chrome user data directory does not exist: ${userDataDirectory}`);
  }
  const profileDirectories = fs.readdirSync(userDataDirectory, { withFileTypes: true }).filter((entry) => {
    return entry.isDirectory() && (entry.name === "Default" || /^Profile \d+$/.test(entry.name));
...
function listChromeProfileDirectories(userDataDirectory) {
  ...
  return fs.readdirSync(userDataDirectory, { withFileTypes: true }).filter((entry) => {
    return entry.isDirectory() && (entry.name === "Default" || /^Profile \d+$/.test(entry.name)) && isUsableChromeProfile(userDataDirectory, entry.name);
  }).map((entry) => entry.name).sort(compareChromeProfileDirectories);
}
function getChromeExtensionInstallStatusForProfile(userDataDirectory, profileDirectory, extensionId, profilePathOverride) {
  const profilePath = profilePathOverride ?? path.join(userDataDirectory, profileDirectory);
  ...
  const versions = fs.existsSync(extensionPath) && fs.statSync(extensionPath).isDirectory() ? fs.readdirSync(extensionPath, { withFileTypes: true }).filter((entry) => entry.isDirectory()).map((entry) => entry.name).sort() : [];
  const unpackedInstalled = preferences.path !== null && fs.existsSync(preferences.path) && fs.statSync(preferences.path).isDirectory();
...
function getChromeExtensionInstallStatus() {
  ...
  const userDataDirectory = preferencesPathOverride == null ? resolveChromeUserDataDirectory() : path.dirname(path.dirname(preferencesPathOverride));
  const selectedProfilePath = preferencesPathOverride == null ? null : path.dirname(preferencesPathOverride);
  const selectedProfileDirectory = selectedProfilePath == null ? resolveChromeProfileDirectory(userDataDirectory) : path.basename(selectedProfilePath);
  const profileDirectories = preferencesPathOverride == null ? listChromeProfileDirectories(userDataDirectory) : [selectedProfileDirectory];
  if (profileDirectories.length === 0) {
    throw new Error(`Could not find a Chrome profile directory with Preferences in ${userDataDirectory}.`);
  }
  const profiles = profileDirectories.map((profileDirectory) => {
    const profileStatus = getChromeExtensionInstallStatusForProfile(userDataDirectory, profileDirectory, extensionId, profileDirectory === selectedProfileDirectory && selectedProfilePath !== null ? selectedProfilePath : undefined);
    return {
      ...profileStatus,
...
function getChromeExtensionPreferences(profilePath, extensionId) {
  ...
  for (const preferencesPath of preferencesPaths) {
    ...
    if (!isRecord(settings))
      continue;
    const extensionSettings = settings[extensionId];
...
function getDisableReasons(disableReasons) {
  if (Array.isArray(disableReasons))
...
function isUsableChromeProfile(userDataDirectory, profileDirectory) {
  if (typeof profileDirectory !== "string" || profileDirectory.length === 0)
    return false;
  return fs.existsSync(path.join(userDataDirectory, profileDirectory, "Preferences"));
}
...
function main() {
  ...
  if (json)
    ...
    console.log(`Selected Chrome profile: ${result.selectedProfileDirectory ?? "none"}`);
    ...
    for (const profile of result.profiles) {
      console.log("");
      console.log(`Profile: ${profile.profileDirectory}${profile.selected ? " (selected)" : ""}`);
      ...
      console.log(`  Registered in Chrome preferences: ${profile.registered ? "yes" : "no"}`);
