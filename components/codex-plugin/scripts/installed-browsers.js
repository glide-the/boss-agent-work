...
var DEFAULT_BROWSER_SCHEMES = ["http", "https"];
...
function readBundleInfo(appPath) {
  ...
  return {
    ...
    CFBundleShortVersionString: readPlistKey(infoPath, "CFBundleShortVersionString")
  };
}
function knownBrowserByBundleId() {
  ...
  for (const browser of KNOWN_BROWSERS)
    for (const bundleId of browser.bundleIds)
      byBundleId.set(bundleId, browser);
  return byBundleId;
}
...
function addMacosApp(installedByBundleId, browser, appPath, fallbackBundleId = null) {
  if (!fs.existsSync(appPath))
    return;
  const info = readBundleInfo(appPath);
  const bundleId = stringProperty(info, "CFBundleIdentifier") || fallbackBundleId || browser.bundleIds[0];
  if (!bundleId)
    return;
  installedByBundleId.set(bundleId, {
    ...
    bundle_id: bundleId,
    path: appPath,
    version: stringProperty(info, "CFBundleShortVersionString") || stringProperty(info, "CFBundleVersion")
  });
}
function findMacosApps() {
  ...
  return [...installedByBundleId.values()].sort((a, b) => a.name.localeCompare(b.name));
}
function resolveMacosAppByBundleId(bundleId) {
  for (const appPath of mdfindAppsForBundleId(bundleId)) {
    ...
    if (discoveredBundleId && discoveredBundleId !== bundleId)
      continue;
    return {
      name: stringProperty(info, "CFBundleName") || stringProperty(info, "CFBundleDisplayName") || path.basename(appPath, ".app"),
      bundle_id: bundleId,
      path: appPath,
      version: stringProperty(info, "CFBundleShortVersionString") || stringProperty(info, "CFBundleVersion")
    };
...
function findDefaultBrowserMacos(installed) {
  const byBundleId = new Map(installed.filter((item) => item.bundle_id).map((item) => [item.bundle_id, item]));
  const knownByBundleId = knownBrowserByBundleId();
  const schemes = {};
  for (const handler of loadLaunchServicesHandlers()) {
    ...
    if (scheme !== "http" && scheme !== "https")
      continue;
    const bundleId = stringProperty(handler, "LSHandlerRoleAll") || stringProperty(handler, "LSHandlerRoleViewer");
    if (!bundleId)
      continue;
    const installedMatch = byBundleId.get(bundleId);
    const knownMatch = knownByBundleId.get(bundleId);
    const resolvedMatch = installedMatch || (knownMatch ? null : resolveMacosAppByBundleId(bundleId));
    schemes[scheme] = {
      scheme,
      bundle_id: bundleId,
      name: installedMatch?.name || knownMatch?.name || resolvedMatch?.name || null,
...
function printTextReport(inventory, check) {
  ...
  else if (defaultBrowser.prog_id)
    console.log(`  ${defaultBrowser.prog_id}`);
  else
    console.log(`  unknown (source: ${defaultBrowser.source || "unknown"})`);
  console.log("");
...
function main() {
  ...
  if (args.json)
    ...
    printTextReport(inventory, args.check);
